...
def build_broker_url():
    ...
    2. BROKER_URL
    3. INVENIO_RABBITMQ_* specific environment variables
    4. Default URL
    ...
    borker_url = os.environ.get("INVENIO_BROKER_URL") or os.environ.get("BROKER_URL")
    if borker_url:
        return borker_url

    broker_params = _get_env_var(
        "INVENIO_RABBITMQ", ["user", "password", "host", "port", "protocol"]
    )
    if all(broker_params.values()):
        vhost = f"{os.environ.get("INVENIO_RABBITMQ_VHOST").lstrip("/")}"
        amq_url = f"{broker_params['protocol']}://{broker_params['user']}:{broker_params['password']}@{broker_params['host']}:{broker_params['port']}/{vhost}"
        return amq_url
    return default_url
...
def build_redis_url(db=None):
    ...
    2. CACHE_REDIS_URL
    3. INVENIO_CACHE_REDIS_* specific environment variables
    4. Default URL
    """
    db = db if db is not None else 0
    default_url = f"redis://localhost:6379/{db}"

    redis_url = os.environ.get("INVENIO_CACHE_REDIS_URL") or os.environ.get(
        "CACHE_REDIS_URL"
    )
    ...
    redis_params = _get_env_var(
        "INVENIO_CACHE_REDIS", ["host", "port", "password", "protocol"]
    )
